...
function processFile(filePath) {
  ...
  // Ignore Generator.tsx and OrderPage.tsx as they are already perfect based on previous prompt
  if (filePath.includes('Generator.tsx') || filePath.includes('OrderPage.tsx')) return;
  ...
  content = content.replace(/#08080A/gi, '#FAFAFA');
  content = content.replace(/#121216/gi, '#FFFFFF');
  content = content.replace(/#D4AF37/gi, '#F4C2C2');
  content = content.replace(/#E4E2DD/gi, '#4A4A4A');
  // Be careful with replacing #FFFFFF to #333333 everywhere as it might mess up threejs colors,
  // Let's rely on the tailwind class replacement for text-white and text-[#FFFFFF] above.
